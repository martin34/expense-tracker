...
namespace finance {
class PartnerNameFilter : public ICategoryFilter {
public:
  PartnerNameFilter(const std::regex &partner_name_pattern,
  ...
  HasCategory(const Transaction &transaction) const {
    if (std::regex_search(transaction.partner_name, partner_name_pattern_)) {
      return {category_};
    }
    return {};
  }
  virtual ~PartnerNameFilter() = default;
